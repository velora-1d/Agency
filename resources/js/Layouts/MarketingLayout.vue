<template>
  <div class="space-y-6">
    <section class="relative overflow-hidden rounded-[2rem] border border-stone-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(247,244,239,0.96)_100%)] p-3 shadow-[0_24px_60px_rgba(28,25,23,0.08)]">
      <div class="absolute inset-x-10 top-0 h-16 rounded-full bg-rose-100/35 blur-3xl"></div>
      <div class="absolute -right-12 top-8 h-24 w-24 rounded-full bg-amber-100/35 blur-3xl"></div>

      <div class="relative space-y-3">
        <div class="grid gap-2 rounded-[1.5rem] bg-stone-950 p-2 shadow-[0_16px_40px_rgba(28,25,23,0.28)] sm:grid-cols-2 xl:grid-cols-4">
          <Link
            v-for="item in items"
            :key="item.key"
            :href="item.href"
            class="inline-flex w-full items-center justify-center gap-2 rounded-[1.1rem] px-4 py-3 text-sm font-semibold transition-all duration-300"
            :class="item.key === activeItem
              ? 'bg-white text-stone-950 shadow-[0_10px_24px_rgba(255,255,255,0.22)]'
              : 'text-stone-300 hover:bg-white/10 hover:text-white'"
          >
            <component :is="item.icon" class="h-4 w-4" />
            <span>{{ item.label }}</span>
          </Link>
        </div>

        <div class="grid gap-4 rounded-[1.35rem] border border-stone-200/80 bg-white/92 px-4 py-4 shadow-[0_14px_34px_rgba(28,25,23,0.08)] backdrop-blur lg:grid-cols-[18rem_minmax(0,1fr)] lg:items-center">
          <div class="min-w-0">
            <p class="text-[10px] font-bold uppercase tracking-[0.24em] text-stone-400">Pemasaran</p>
            <p class="mt-1 text-sm font-semibold text-stone-900">{{ activeMeta.description }}</p>
          </div>

          <div class="flex flex-wrap items-stretch gap-2 lg:justify-end">
            <div
              v-for="pill in activeMeta.pills"
              :key="pill"
              class="inline-flex min-w-[8.5rem] items-center justify-center rounded-full border border-stone-200 bg-stone-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-stone-500"
            >
              {{ pill }}
            </div>
          </div>
        </div>
      </div>
    </section>

    <slot />
  </div>
</template>

<script setup>
import { computed } from 'vue'
import { Link, usePage } from '@inertiajs/vue3'
import { BarChart3, Mail, Megaphone, ScrollText } from 'lucide-vue-next'

const props = defineProps({
  workspace: {
    type: Object,
    required: true,
  },
})

const page = usePage()
const baseUrl = `/w/${encodeURIComponent(props.workspace.slug)}/marketing`

function tabUrl(tab) {
  return `${baseUrl}?tab=${tab}`
}

const items = computed(() => [
  {
    key: 'overview',
    label: 'Ringkasan',
    icon: Megaphone,
    href: tabUrl('overview'),
    description: 'Ringkasan kampanye aktif, biaya, prospek, ROI, dan konten yang paling kuat.',
    pills: ['Kampanye', 'Anggaran', 'Prospek', 'ROI'],
  },
  {
    key: 'social',
    label: 'Sosial',
    icon: ScrollText,
    href: tabUrl('social'),
    description: 'Perencana konten lintas platform, status alur, jadwal rilis, dan performa per unggahan.',
    pills: ['Perencana', 'Platform', 'Jadwal', 'Keterlibatan'],
  },
  {
    key: 'email',
    label: 'Email',
    icon: Mail,
    href: tabUrl('email'),
    description: 'Kelola buletin, audiens, jadwal pengiriman, dan kesehatan daftar email workspace.',
    pills: ['Kampanye', 'Audiens', 'Pengiriman', 'Laju'],
  },
  {
    key: 'analytics',
    label: 'Analitik',
    icon: BarChart3,
    href: tabUrl('analytics'),
    description: 'Lihat trafik, prospek, performa email, dan rincian sumber dari data pemasaran yang masuk.',
    pills: ['Trafik', 'Sumber', 'Email', 'Konversi'],
  },
])

const activeItem = computed(() => {
  const query = new URLSearchParams(page.url.split('?')[1] ?? '')
  return query.get('tab') || 'overview'
})

const activeMeta = computed(() => {
  return items.value.find((item) => item.key === activeItem.value) ?? items.value[0]
})
</script>

<template>
  <div class="group relative flex gap-x-4">
    ...
    <div class="flex-auto rounded-4xl border border-stone-200 bg-white p-5 shadow-[0_10px_30px_rgba(60,42,24,0.04)] transition-all duration-300 hover:shadow-[0_20px_50px_rgba(60,42,24,0.08)] hover:-translate-y-0.5">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div v-if="activity.type" class="self-start flex flex-col items-end gap-2">
          <span 
      ...
      <div class="relative">
        ...
        <div 
          ...
          class="absolute bottom-full left-11 z-50 mb-2 w-64 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-2xl animate-reveal"
        >
          <div class="bg-stone-50 px-4 py-2 border-b border-stone-100">
        ...
        <form @submit.prevent="submitComment" class="mt-6 flex items-start gap-3">
          ...
          <div class="relative flex-auto">
            ...
            <p v-if="form.errors.content" class="mt-1 text-xs text-rose-500 px-2">{{ form.errors.content }}</p>
          </div>
        </form>
      </div>
    </div>
...
function insertMention(user) {
  ...
  const lastAtSymbol = textBeforeCursor.lastIndexOf('@')
  const newContent = textBeforeCursor.substring(0, lastAtSymbol) + `@${user.name} ` + textAfterCursor
...
function getSubjectRoute(activity) {
  ...
  try {
    switch (type) {
      ...
      case 'invoice':
        return route('workspace.finance.overview', { workspace: workspace.slug }) // Partial link
      case 'task':
        // Task detail might be inside project
        return route('workspace.projects.index', { workspace: workspace.slug }) // Fallback
      case 'meeting':
